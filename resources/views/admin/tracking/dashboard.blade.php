...
@section('content')
    <div class="luxury-tracking-dashboard-page">
        <!-- Hero Section -->
        <x-admin-hero icon="fas fa-chart-line" title="Tracking Dashboard" subtitle="Real-time order tracking and management"
        ...
        <x-admin-responsive-grid class="stats-section auto-fit" :delay="300">
            <x-admin-stat-card icon="fas fa-shopping-cart" :value="$stats['total_orders']" label="Total Orders"
                change="+{{ $stats['today_orders'] }} today" changeType="positive" iconType="primary" :delay="400" />
            <x-admin-stat-card icon="fas fa-clock" :value="$stats['pending_orders'] + $stats['processing_orders'] + $stats['shipping_orders']" label="Active Orders" change="{{ $stats['pending_orders'] }} pending"
                changeType="warning" iconType="warning" :delay="500" />
            <x-admin-stat-card icon="fas fa-truck" :value="$stats['active_drivers']" label="Active Drivers"
                change="{{ $stats['active_drivers'] }}/{{ $stats['total_drivers'] }} available" changeType="positive"
                iconType="success" :delay="600" />
            <x-admin-stat-card icon="fas fa-check-circle" :value="$stats['on_time_delivery_rate'] . '%'"
                label="On-Time Rate" change="{{ $stats['avg_delivery_time'] }} min avg" changeType="positive"
                iconType="info" :delay="700" />
        </x-admin-responsive-grid>

        <div class="tracking-dashboard-grid">
            <!-- Recent Orders -->
            <x-admin-content-card title="Recent Orders" icon="fas fa-list" :delay="800" class="recent-orders-section">
                ...
                <div class="orders-container" id="recent-orders-container">
                    @foreach($recentOrders as $order)
                        <div class="order-item fade-in-up delay-{{ $loop->index * 100 }}" data-status="{{ $order->status }}"
                            onclick="window.location.href='{{ route('tracking.show', $order->tracking_code) }}'"
                            ...
                            <div class="order-header">
                                ...
                                <div class="order-actions" onclick="event.stopPropagation()">
                                    <button onclick="viewOrderTimeline({{ $order->id }})" class="action-btn-icon timeline"
                                        title="View Timeline">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <button onclick="updateOrderStatus({{ $order->id }})" class="action-btn-icon update"
                                        title="Update Status">
                            ...
                            <div class="order-details">
                                <div class="customer-info">
                                    <span class="customer-name"><i
                                            class="fas fa-user mr-1"></i>{{ $order->recipient_name }}</span>
                                    <span class="customer-phone"><i
                                            class="fas fa-phone mr-1"></i>{{ $order->recipient_phone }}</span>
                                    <span class="order-time"><i
                                            class="fas fa-calendar-alt mr-1"></i>{{ $order->created_at->format('d M, H:i') }}</span>
                                </div>
                                @if($order->driver)
            ...
            <div class="tracking-sidebar">
                <!-- Active Drivers -->
                <x-admin-content-card title="Active Drivers" icon="fas fa-truck" :delay="900">
                ...
                <!-- Recent Activity -->
                <x-admin-content-card title="Recent Activity" icon="fas fa-history" :delay="1000">
                ...
                <!-- Performance Metrics -->
                <x-admin-content-card title="Performance Metrics" icon="fas fa-chart-bar" :delay="1100">
                    <div class="metrics-container">
                        ...
                        <div class="metric-item">
                            ...
                            <span class="metric-value">Rp
                                {{ number_format($performanceMetrics['revenue_last_7_days']) }}</span>
                        </div>
                        <div class="metric-item">
                        ...
                        <div class="metric-item">
                            ...
                            <span
                                class="metric-value">{{ number_format($performanceMetrics['customer_satisfaction'], 1) }}/5</span>
                        </div>
                        <div class="metric-item">
                ...
                <!-- Notification Stats -->
                <x-admin-content-card title="Notification Stats" icon="fas fa-bell" :delay="1200">
                    <div class="notification-stats-container" id="notification-stats-container">
                        ...
                        <div class="notification-stat-item">
                            ...
                            <span class="stat-value success"
                                id="notification-sent">{{ $notificationStats['sent'] ?? 0 }}</span>
                        </div>
                        <div class="notification-stat-item">
                            <span class="stat-label">Delivered</span>
                            <span class="stat-value info"
                                id="notification-delivered">{{ $notificationStats['delivered'] ?? 0 }}</span>
                        </div>
                        <div class="notification-stat-item">
                            <span class="stat-label">Failed</span>
                            <span class="stat-value error"
                                id="notification-failed">{{ $notificationStats['failed'] ?? 0 }}</span>
                        </div>
                        <div class="notification-stat-item">
                            <span class="stat-label">Pending</span>
                            <span class="stat-value warning"
                                id="notification-pending">{{ $notificationStats['pending'] ?? 0 }}</span>
                        </div>
                    </div>
                </x-admin-content-card>
            </div>
    ...
    <script>
        ...
        function updateRecentOrders(orders) {
            ...
            orders.forEach(order => {
                ...
                orderElement.onclick = () => window.location.href = `/tracking/${order.tracking_code}`;

                const timestamp = new Date(order.created_at);
                const formattedDate = timestamp.toLocaleDateString('id-ID', { day: '2-digit', month: 'short' }) + ', ' +
